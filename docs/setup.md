...
2. Copy `.env.example` to `.env` and fill in `APP_ENV`, Google OAuth (local/prod), admin email, Garmin encryption key, and OpenAI credentials.
...
5. Set `OPENAI_API_KEY` and, if you want to override the default model, `OPENAI_MODEL_NAME` (defaults to `gpt-5-mini`).
