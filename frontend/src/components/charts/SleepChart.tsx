...
export function SleepChart({ variant = 'card' }: ChartProps) {
  ...
  const tooltipRenderer = ({ active, payload }: TooltipProps<number, string>) => {
    ...
    return <TooltipLabel $color={tooltipLabelColor}>{`${value.toFixed(2)} hrs`}</TooltipLabel>;
  };
  const content = (
    <>
      ...
      <ResponsiveContainer width="100%" height={240}>
        <LineChart data={bars} margin={{ top: 12, right: 12, left: 16, bottom: 22 }}>
          <CartesianGrid stroke={appTheme.colors.grid} strokeDasharray="1 10" opacity={0.3} />
          <XAxis
            dataKey="timestamp"
            axisLine={false}
            tickLine={false}
